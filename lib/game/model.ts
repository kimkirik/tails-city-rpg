...
export type Action={type:'interact';id:string}|{type:'attack'|'skill'|'tail'|'flee'|'expand'|'pickup'}|{type:'item'|'buy';id:string}|{type:'switch'|'party';id:string}|{type:'rename';name:string}|{type:'travel';region:number;gate?:boolean;edge?:number};
...
export function act(source:GameState,a:Action):Result {const s=structuredClone(source);let message='';let event:Result['event'];let loot:Result['loot'];const fail=(m:string)=>({state:source,message:m});
 ...
 if(a.type==='attack'||a.type==='skill'||a.type==='tail'){if(!s.battle)return fail('전투 중에 사용할 수 있어요.');if(a.type==='skill'&&s.battle.cooldown>0)return fail(`${s.battle.cooldown}턴 뒤에 기술을 쓸 수 있어요.`);const d=currentDog(s);const support=partyDogs(s).find(p=>p.id!==d.id&&p.hp>0);const assist=support?Math.max(1,Math.round(support.atk*(a.type==='skill'?.8:.65))):0;const primary=d.atk+(a.type==='skill'?18+d.level*2:7+(s.weapon?WEAPONS[s.weapon].attack:0));const damage=primary+assist;s.battle.enemy.hp=Math.max(0,s.battle.enemy.hp-damage);s.enemyHealth[s.battle.enemy.id]=s.battle.enemy.hp;message=`${d.name}의 ${a.type==='skill'?BREEDS[d.breed].skill:a.type==='tail'?'회전 꼬리치기':'돌진 공격'}! ${primary} 피해${support?` · ${support.name} 지원 +${assist}`:''}`;s.battle.log.push(message);event='attack';if(a.type==='skill')s.battle.cooldown=3;
