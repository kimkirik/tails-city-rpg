import {defaultAppearance,validAppearance,type Appearance} from './appearance.ts';
...
export function combatHits(s:GameState,kind:'attack'|'tail'|'skill'){
 const lead=currentDog(s);const hits=[{actorId:'traveler',slot:-1,damage:7+(s.weapon?WEAPONS[s.weapon].attack:0)},{actorId:lead.id,slot:0,damage:lead.atk+(kind==='skill'?11+lead.level*2:0)}];
 const support=partyDogs(s).find(d=>d.id!==lead.id&&d.hp>0);
 if(support)hits.push({actorId:support.id,slot:1,damage:Math.max(1,Math.round(support.atk*(kind==='skill'?.8:.65)))});
 return hits;
}
export type Action={type:'interact';id:string}|{type:'attack'|'skill'|'tail'|'flee'|'expand'|'pickup'}|{type:'item'|'buy';id:string}|{type:'switch'|'party';id:string}|{type:'rename';name:string}|{type:'dress';appearance:Appearance}|{type:'travel';region:number;gate?:boolean;edge?:number};
...
export function act(source:GameState,a:Action):Result {const s=structuredClone(source);let message='';let event:Result['event'];let loot:Result['loot'];const fail=(m:string)=>({state:source,message:m});
 ...
 if(a.type==='dress'){if(s.battle)return fail('전투가 끝나면 옷을 갈아입을 수 있어요.');if(!validAppearance(a.appearance))return fail('꾸미기 정보를 확인해 주세요.');s.appearance={...a.appearance};return {state:s,message:'새로운 모습으로 갈아입었어요!'};}
 ...
 if(a.type==='attack'||a.type==='skill'||a.type==='tail'){if(!s.battle)return fail('전투 중에 사용할 수 있어요.');if(a.type==='skill'&&s.battle.cooldown>0)return fail(`${s.battle.cooldown}턴 뒤에 기술을 쓸 수 있어요.`);const d=currentDog(s);const hits=combatHits(s,a.type);const damage=hits.reduce((n,hit)=>n+hit.damage,0);s.battle.enemy.hp=Math.max(0,s.battle.enemy.hp-damage);s.enemyHealth[s.battle.enemy.id]=s.battle.enemy.hp;message=`${s.playerName} + ${d.name}의 ${a.type==='skill'?BREEDS[d.breed].skill:a.type==='tail'?'회전 꼬리치기':'협공'}! ${hits.map(hit=>`${hit.actorId==='traveler'?s.playerName:s.dogs.find(dog=>dog.id===hit.actorId)!.name} ${hit.damage}`).join(' · ')} 피해`;s.battle.log.push(message);event='attack';if(a.type==='skill')s.battle.cooldown=3;
...
export function unpackSave(text:string):GameState {if(text.length>200000)throw Error('저장 파일이 너무 큽니다.');const p=JSON.parse(text);const s=p?.state;const int=(v:unknown,min:number,max:number)=>Number.isInteger(v)&&Number(v)>=min&&Number(v)<=max;const ids=(v:unknown,re:RegExp,max:number)=>Array.isArray(v)&&v.length<=max&&new Set(v).size===v.length&&v.every(x=>typeof x==='string'&&re.test(x));if(p.game!=='tails-city'||!s||s.version!==1||!int(s.region,0,5)||!Number.isFinite(s.x)||!Number.isFinite(s.y)||!walkable(s.x,s.y,s.region,s.place??'field')||!int(s.coins,0,99999999)||!int(s.capacity,25,100)||(s.capacity-25)%5!==0||!Array.isArray(s.bag)||s.bag.length!==s.capacity||!s.bag.every((v:Slot)=>v===null||(v&&ITEMS[v.item]&&int(v.qty,1,9)))||!Array.isArray(s.dogs)||s.dogs.length<1||s.dogs.length>13||!s.dogs.every((d:Dog)=>d&&typeof d.id==='string'&&/^(starter|dog-[0-5](-b)?)$/.test(d.id)&&typeof d.name==='string'&&d.name.length>0&&d.name.length<=20&&int(d.breed,0,BREEDS.length-1)&&(d.sex===undefined||d.sex==='female'||d.sex==='male')&&(d.coat===undefined||d.coat==='brown')&&int(d.level,1,999)&&int(d.xp,0,999999)&&int(d.hp,0,d.maxHp)&&int(d.maxHp,1,99999)&&int(d.atk,1,99999)&&int(d.bond,0,999999))||new Set(s.dogs.map((d:Dog)=>d.id)).size!==s.dogs.length||!s.dogs.some((d:Dog)=>d.id===s.active)||!ids(s.defeated,/^(enemy-[0-5]-[01]|captain-[0-5])$/,18)||!ids(s.recruited,/^dog-[0-5](-b)?$/,12)||!ids(s.looted,/^loot-[0-5](-b)?$/,12)||!Array.isArray(s.visited)||!s.visited.includes(s.region)||!s.visited.every((r:number)=>int(r,0,5))||!int(s.steps,0,999999999)||!int(s.seconds,0,999999999)||typeof s.won!=='boolean')throw Error('테일즈 시티 저장 파일 형식이 올바르지 않습니다.');
 const appearance=s.appearance??defaultAppearance();if(!validAppearance(appearance))throw Error('여행자의 꾸미기 정보가 올바르지 않습니다.');
 ...
 return {...s,appearance:{...appearance},playerName,party,drops,place,weapon,enemyHealth,respawnAt,kills:s.kills??0,encounterGraceUntil:s.encounterGraceUntil??0,dogs:s.dogs.map((dog:Dog)=>dog.id==='starter'?asRichi(dog):dog),battle:null} as GameState;
}
