...
export function entities(s:GameState):Entity[]{const r=s.region;if(s.place==='shop')return [{id:`counter-${r}`,kind:'merchant',name:'상인 미로',x:1024,y:820},{id:`exit-${r}`,kind:'exit',name:'상점 출구',x:1024,y:1840}];const names=['구름','보리','번개','초코','두부','루나'];return [
 ...
 ].filter(e=>(r!==1||e.kind!=='enemy')&&!s.defeated.includes(e.id)&&!s.recruited.includes(e.id)&&!s.looted.includes(e.id)&&(!(e.id in s.respawnAt)||s.respawnAt[e.id]<=s.seconds)) as Entity[];}
...
export type Result={state:GameState;message:string;event?:'shop'|'win'|'loss'|'recruit'|'attack'|'enter_shop'|'leave_shop'};
export function act(source:GameState,a:Action):Result {const s=structuredClone(source);let message='';let event:Result['event'];const fail=(m:string)=>({state:source,message:m});
 ...
 if(e.kind==='enemy'){if(!s.dogs.some(d=>d.hp>0))return fail('동료들이 지쳤어요. 쉼터에서 쉬어 주세요.');if(e.id==='captain-5'&&s.defeated.filter(id=>id.startsWith('captain-')).length<5)return fail('마을 밖의 다섯 대장을 먼저 쓰러뜨리고 신호기를 꺼야 해요.');if(currentDog(s).hp<=0)s.active=s.dogs.find(d=>d.hp>0)!.id;s.battle={enemy:enemyStats(s,e),turn:1,cooldown:0,log:[`${e.name}(이)가 길을 막아섰다!`]};message='동료와 힘을 합쳐 싸우세요.';}
 }
 ...
 if(a.type==='item'&&!WEAPONS[a.id]){if(!ITEMS[a.id])return fail('알 수 없는 아이템입니다.');if(a.id==='treat')return fail('강아지 가까이에서 E를 누르면 간식을 줄 수 있어요.');const d=currentDog(s);if(d.hp===d.maxHp)return fail('이미 체력이 가득 차 있어요.');if(d.hp<=0&&a.id!=='revive')return fail('쓰러진 동료에게는 든든한 도시락을 사용해 주세요.');if(!takeItem(s,a.id))return fail('아이템이 없어요.');d.hp=Math.min(d.maxHp,d.hp+(a.id==='potion'?45:a.id==='berry'?20:d.maxHp));message=`${d.name}(이)의 체력이 회복되었어요.`;if(s.battle){s.battle.log.push(message);enemyTurn(s);}}
 if(a.type==='attack'||a.type==='skill'){if(!s.battle)return fail('전투 중에 사용할 수 있어요.');if(a.type==='skill'&&s.battle.cooldown>0)return fail(`${s.battle.cooldown}턴 뒤에 기술을 쓸 수 있어요.`);const d=currentDog(s);const damage=d.atk+(a.type==='skill'?18+d.level*2:7+(s.weapon?WEAPONS[s.weapon].attack:0));s.battle.enemy.hp=Math.max(0,s.battle.enemy.hp-damage);s.enemyHealth[s.battle.enemy.id]=s.battle.enemy.hp;message=`${d.name}의 ${a.type==='skill'?BREEDS[d.breed].skill:'함께 공격'}! ${damage} 피해`;s.battle.log.push(message);event='attack';if(a.type==='skill')s.battle.cooldown=3;
 if(s.battle.enemy.hp<=0){const en=s.battle.enemy;const reward=55+en.region*25+(en.captain?100:0);s.coins+=reward;s.kills++;if(en.id.startsWith('mob-'))s.respawnAt[en.id]=s.seconds+30;else s.defeated.push(en.id);d.xp+=35+en.region*15+(en.captain?30:0);d.bond++;while(d.xp>=d.level*45){d.xp-=d.level*45;d.level++;d.maxHp+=10;d.atk+=3;d.hp=d.maxHp;}s.battle=null;message=`승리! +${reward} 코인 · ${d.name} Lv.${d.level}`;event='win';if(en.id==='captain-5'){s.won=true;message='도시를 되찾았어요! 강아지들과 함께 여섯 지역을 자유롭게 탐험하세요.';}}
 else enemyTurn(s);
 }
...
export function unpackSave(text:string):GameState {if(text.length>200000)throw Error('저장 파일이 너무 큽니다.');const p=JSON.parse(text);const s=p?.state;const int=(v:unknown,min:number,max:number)=>Number.isInteger(v)&&Number(v)>=min&&Number(v)<=max;const ids=(v:unknown,re:RegExp,max:number)=>Array.isArray(v)&&v.length<=max&&new Set(v).size===v.length&&v.every(x=>typeof x==='string'&&re.test(x));if(p.game!=='tails-city'||!s||s.version!==1||!int(s.region,0,5)||!Number.isFinite(s.x)||!Number.isFinite(s.y)||!walkable(s.x,s.y,s.region,s.place??'field')||!int(s.coins,0,99999999)||!int(s.capacity,25,100)||(s.capacity-25)%5!==0||!Array.isArray(s.bag)||s.bag.length!==s.capacity||!s.bag.every((v:Slot)=>v===null||(v&&ITEMS[v.item]&&int(v.qty,1,9)))||!Array.isArray(s.dogs)||s.dogs.length<1||s.dogs.length>13||!s.dogs.every((d:Dog)=>d&&typeof d.id==='string'&&/^(starter|dog-[0-5](-b)?)$/.test(d.id)&&typeof d.name==='string'&&d.name.length>0&&d.name.length<=20&&int(d.breed,0,BREEDS.length-1)&&(d.sex===undefined||d.sex==='female'||d.sex==='male')&&(d.coat===undefined||d.coat==='brown')&&int(d.level,1,999)&&int(d.xp,0,999999)&&int(d.hp,0,d.maxHp)&&int(d.maxHp,1,99999)&&int(d.atk,1,99999)&&int(d.bond,0,999999))||new Set(s.dogs.map((d:Dog)=>d.id)).size!==s.dogs.length||!s.dogs.some((d:Dog)=>d.id===s.active)||!ids(s.defeated,/^(enemy-[0-5]-[01]|captain-[0-5])$/,18)||!ids(s.recruited,/^dog-[0-5](-b)?$/,12)||!ids(s.looted,/^loot-[0-5](-b)?$/,12)||!Array.isArray(s.visited)||!s.visited.includes(s.region)||!s.visited.every((r:number)=>int(r,0,5))||!int(s.steps,0,999999999)||!int(s.seconds,0,999999999)||typeof s.won!=='boolean')throw Error('테일즈 시티 저장 파일 형식이 올바르지 않습니다.');
 ...
 return {...s,place,weapon,enemyHealth,respawnAt,kills:s.kills??0,encounterGraceUntil:s.encounterGraceUntil??0,dogs:s.dogs.map((dog:Dog)=>dog.id==='starter'?asRichi(dog):dog),battle:null} as GameState;
}
