...
export default function BattleView({state,clip,onAction,onBag,onParty}:{state:GameState;clip:PlayingClip|null;onAction:(a:Action)=>unknown;onBag:()=>void;onParty:()=>void}){
 const [clock,setClock]=useState({id:0,elapsed:0});const elapsed=clock.id===clip?.id?clock.elapsed:0;
 useEffect(()=>{if(!clip)return;let frame=0,last=0;const tick=(time:number)=>{if(time-last>32){setClock({id:clip.id,elapsed:Math.max(0,time-clip.startedAt)});last=time;}frame=requestAnimationFrame(tick);};frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame);},[clip]);
 const battle=state.battle;if(!battle)return null;
 const support=clip?.strikes.find(hit=>hit.slot===1);
 ...
 return <div className={`battle-screen touch-battle action-battle ${busy?'is-acting':''}`}>
  ...
  <div className="combat-actions action-buttons" aria-busy={busy}>
   <button disabled={busy} onClick={()=>onAction({type:'attack'})}><Swords/><span>함께 공격</span><small>{state.playerName} + {lead.name}</small></button>
